...
public class Mesh : MonoBehaviour
{
    ...
    void UpdateMesh()
    {
        ...
        if (ct != playerData.charactorType)     //Mesh가 바뀌면 감지
        {
            for (int i = 0; i < PlayerMesh.Length; i++)         //Deactive Mesh
            {
                PlayerMesh[i].SetActive(false);
            }
            if(!playerData.diee)            //플레이어 Mesg설정
            {
                switch (playerData.charactorType)
                {
                    case CharType.포도:
    ...
    public void VictoryAnimation(bool Active)
    {
        animator.SetBool("Victory", Active);
    }
}
